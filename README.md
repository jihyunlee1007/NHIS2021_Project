...
We import necessary modules from PyQt5: QMainWindow, QPushButton, QWidget, QHBoxLayout, QVBoxLayout, QLabel, QSpacerItem, QSizePolicy, Qt, QFont, and QPixmap. We also import the modules: ChiSquare2Var, DataSet, ThreeDGraph, and TwoDGraph. The urllib.request library is imported as well.
...
- ***def init_ui(self):***
In this method we essentially initalize the user interface of the main menu window. First we set the window title to "2021 National Health Interview Survey (NHIS) 2D GUI" and resize the window to be 1900 by 1000 pixels. Then in order to add in the NHIS logo, we obtain the url of the image file and open a connection to this specific url address using the *urllib.request.urlopen()* function from the *urllib.request* library. Then we read in the content of the image file and create an instance of this image using the *QPixmap()* method from the *QPixmap* library. Next we load the image into the *QPixmap()* object which successfully loads the image from the url address. A label widget is then created to add the image onto the main menu window. We set the height of the image to be 70 pixels while maintaining the image's ratios. *setPixmap()* is then called to set the image that is going to be displayed as the scaled pixmap within a widgit. Next we create the title label called "CDC National Health Interview Survey 2021 Data Analysis" for our main menu window and align it to be centered. Next we set the font size to 28, the color to hex #252547, and bold it. Then we create a layout instance which will be used to arrange the image label and the title label side by side horizontally. The image label widget is then added to the title layout. From here we call *setAlignment()* to align the title layout which contains both the image and the title to be centered horizontally. Then we create a layout instance which sets the layout of the main menu window to verticle. We then align the widgets to the top of the main menu window. Then the title label is added as a widget to the layout. Next, we create a spacer to create some space in between the title label and the buttons that we want to create in the main menu window. Now, we create a layout instance of horizontal and align it to be centered. This will allow us to but all of our buttons next to each other instead of being stacked on top of each other. We then create a push button that connects the button clicking event to the *show_2d_graph()* method. We set the color hex to #4D4D99 and the size of the button to be 450 by 40 pixels with and a font size of 14. Then we add the button to the layout. We repeat the same steps for our *3D Graph* button except we call the *show_3d_graph()* method when the button is clicked. Likewise, we repeat the same steps for our *Chi-Square Analysis (2 Variables)* button except we call the *show_chi_square()* method when the button is clicked. Now we add the button layout to the main layout of the main menu window. This ensures that the buttons are placed under the title label. Then, we change the background color of the main menu window to hex #CCCCCC to give it a slight contrast between the *2D Graph*, *3D Graph*, and *Chi-Square Analysis (2 Variables)* windows.
...
- ***def plot_graph(self):***
In order to ensure precautions, we have the code for this method within a try block. We first clear any existing plots before plotting a new 3D graph. Next, we retrieve the names of the selected variables from the three drop down boxes. If any of the drop down boxes are empty the method returns which basically prevents the method from plotting the graph. Next, we set our respective x, y, and z variables using their corresponding data to the dataset. Then, using the *matplotlib* library we set up the 3D graph and set axis-titles as well as the graph title. We specifically set alpha to 0.3 making the datapoints more transparent and allowing for density to show on the graph. The color of these points are also set to red. We then call *.canvas.draw()* which displays the new 3D plot.
...
- ***def plot_graph(self):***
In order to ensure precautions, we have the code for this method within a try block. We first clear any existing plots before plotting a new 2D graph. Next, we retrieve the names of the selected variables from the two drop down boxes. If any of the drop down boxes are empty the method returns which basically prevents the method from plotting the graph. Next, we set our respective x and y variables using their corresponding data to the dataset. Then, using the *matplotlib* library we set up the 2D graph and set axis-titles as well as the graph title. We specifically set alpha to 0.3 making the datapoints more transparent and allowing for density to show on the graph. The color of these points are also set to red. We then call *.canvas.draw()* which displays the new 2D plot.
